...
2. **Three tiers with one policy**: a VRAM tier of ~1,650–2,400 experts at 160K context (up to 3,700 at short context), a pinned RAM arena sized from the memory available (these numbers were taken with ~10 GB; the console now builds 14 GB on the reference machine, worth about +3% of decode per GB) and the NVMe; 91–97% of lookups hit and 57–86% are served from VRAM, depending on the quant and the tier the context leaves.
